...
const assessmentSchema = z.object({
  ...
  role: z.string().min(2, 'Ruolo deve essere specificato'),
  company: z.string().min(2, 'Società deve essere specificata'),
  website: z.string().optional(),
  
  // Assessment Questions
  aiVisionClarity: z.number().min(1).max(5),
  visionFormalized: z.enum(['si', 'no', 'in_parte']),
  aiStrategicImportance: z.enum(['si', 'no', 'parzialmente']),
  competitiveAdvantage: z.number().min(1).max(5),
  investmentPlans: z.enum(['si', 'no']),
  currentProjects: z.enum(['0', '1-3', '3-7', '>7']),
  aiAreas: z.array(z.string()).optional(),
  pilotProjects: z.enum(['si', 'no', 'previsto_a_breve']),
  employeeUsage: z.number().min(1).max(5),
  managementUsage: z.number().min(1).max(5),
  mainChallenges: z.array(z.string()).max(2),
  partnerships: z.enum(['si', 'no', 'previste_a_breve']),
  dataReadiness: z.enum(['si', 'no', 'parzialmente']),
  internalSkills: z.enum(['si', 'no', 'limitate']),
  trainingInitiatives: z.enum(['si', 'no', 'previste_a_breve']),
  decisionMakerAwareness: z.enum(['si', 'no', 'parzialmente']),
  dedicatedTeam: z.enum(['si', 'no', 'previsto_a_breve']),
  aiPolicies: z.enum(['si', 'no', 'parzialmente', 'non_lo_so']),
  aiMetrics: z.enum(['si', 'no', 'in_sviluppo']),
...
Contatta il nostro team per una consultazione gratuita dove approfondiremo i risultati del tuo assessment e svilupperemo insieme una strategia AI su misura per ${data.company}.`
    ...
    const saveResult = await saveAssessment(data, aiSummary)
